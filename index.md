...
- exact cross-fit audits of continuous SOM topology under explicit
  computation budgets;
...
is experimental and descriptive: it does not score, rank or select maps,
and its fit pairs and sample pairs are not treated as independent
replicates.
...
can make only deterministic migrations supported by retained evidence;
it asks for recomputation when a scientifically important field cannot
be recovered. The object contract is not a substitute for the package
version, input provenance or
[`sessionInfo()`](https://rdrr.io/r/utils/sessionInfo.html).
